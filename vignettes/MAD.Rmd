...
The plot can include the regions of interest by

```{r plots}
ans <- read.delim("example_T_9.txt")
example <- addAnnot(example) # add annotation
ss <- "CONTROL191"
plotQMosaic(example, sample=ss, chr=20,
            regions = ans[ans$sample==ss,])
```

We can also obtain a zoom plot visualizing the region of interest can also be zoom out

```{r zoom, fig.height=9}
plotZoomQMosaic(example, sample=ss, chr=20,
            regions = ans[ans$sample==ss,][2,])
```
