...
This function creates an object with the altered regions:
...
Finally, if the user is interested in looking at a given region with regard to B-allele frequency and `log2ratio`, the  `plotQMosaic` function can be used to get the figure \ref{fig-control191chr20}

```{r control191chr20}
example <- addAnnot(example) # add annotation
ss <- "CONTROL191"
plotQMosaic(example, sample=ss, chr=20, 
            regions=mosaics)
```

We can also zoom out the region of interest by:

```{r zoom, fig.height=9}
example <- addAnnot(example)
plotZoomQMosaic(example, sample=ss, chr=20,
            regions = mosaics)
```

We can also visualize the genomic region using `plotCNV` function from `R-GADA` package indicating `mosaic=TRUE`. The function requires annotation from `Homo.sapiens` and the `Gviz` library. Mosaic gains are in green and loses in blue, UPD are in orange, trisomies in tomato and LOH in red

```{r plotGR}
library(Gviz)
library(Homo.sapiens)
rr <- GRanges("chr16:63.5e6-67.0e6")
plotCNVs(mosaics, range=rr, drawGenes = TRUE, mosaic=TRUE)
```
...
unlink("example_T_9.txt", recursive = TRUE)
```
